/// <summary>
/// Page PDC Demand Plan Sales Orders (ID 50067).
/// </summary>
page 50067 "PDC Demand Plan Sales Orders"
{
    ...
    layout
    {
        area(content)
        {
            field(ShowData; ShowData)
            {
                ...
                Caption = 'Show Data';
                Lookup = true;
                OptionCaption = 'All,Sales Orders';
                Visible = false;
    ...
    var
        Item1: Record Item;
    begin
        if ShowData in [0, 1] then begin //All, Sales
            Item1.Get(Rec."No.");
            if ColumnID = 1 then
            ...
            Result += Item1."Qty. on Sales Order";
        end; //Sales
    ...
    begin
        if ShowData in [1] then begin //All, Sales
            SalesLine.SetRange("Document Type", SalesLine."document type"::Order);
            SalesLine.SetRange(Type, SalesLine.Type::Item);
            SalesLine.SetRange("No.", Rec."No.");
            if ColumnID = 1 then
                SalesLine.SetFilter("Shipment Date", '..%1', NormalDate(TempDateMatrixPeriod[ColumnID]."Period End"))
            else
                SalesLine.SetFilter("Shipment Date", '%1..%2', TempDateMatrixPeriod[ColumnID]."Period Start", NormalDate(TempDateMatrixPeriod[ColumnID]."Period End"));
