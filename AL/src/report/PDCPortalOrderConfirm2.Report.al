/// <summary>
/// report PDC Portal - Order Confirm2 (ID 50063).
/// </summary>
report 50063 "PDC Portal - Order Confirm2"
{
    Caption = 'Portal - Order Confirmation2';
    ProcessingOnly = true;
    ...
    begin
        ...
        if SalesLine1.FindFirst() then begin
            PortalUser.SetRange("Contact No.", SalesLine1."PDC Created By ID");
        ...
        SalesSetup.Get();
        if AttRec.Get(SalesSetup."PDC Order Conf2. E-mail Body") then
        ...
        if BodyText <> '' then begin
            ...
            if SalesLine1.FindFirst() then
                if SalesLine1.Findset() then begin
                    PONo := SalesLine1."PDC Customer Reference";
                    repeat
            ...
            MapHTMLFields(BodyText, '{{orderNumber}}', SalesHeader1."No.");
            MapHTMLFields(BodyText, '{{poNumber}}', PONo);
            MapHTMLFields(BodyText, '{{orderDate}}', NAVPortalsMgt.FormatDate(SalesHeader1."Order Date"));
            MapHTMLFields(BodyText, '{{requestedDespatch}}', NAVPortalsMgt.FormatDate(SalesHeader1."Requested Delivery Date")); //21.10.2019 JEMEL J.Jemeljanovs #3043

            if LinesBodyText <> '' then
                MapHTMLFields(BodyText, '{{tableLine}}', LinesBodyText);

            MapHTMLFields(BodyText, '{{deliveryAddress}}',
            ...
            EmailMessage.Create(ListTo, 'Order Confirmation ' + SalesHeader1."No.", BodyText, true, ListCC, ListBCC);
