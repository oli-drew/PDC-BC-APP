/// <summary>
/// TableExtension PDCSalesInvoiceHeader (ID 50019) extends Record Sales Invoice Header.
/// </summary>
tableextension 50019 PDCSalesInvoiceHeader extends "Sales Invoice Header"
{
    fields
    {
        field(50000; "PDC Employee Name"; Text[50])
        {
            Caption = 'Employee Name';
        }
        field(50001; "PDC Customer Reference"; Code[30])
        {
            Caption = 'Customer Reference';
        }
        field(50003; "PDC WebOrder No."; Code[20])
        {
            Caption = 'WebOrder No.';
        }
        field(50004; "PDC Employee ID"; Code[20])
        {
            Caption = 'Employee ID';
        }
        field(50006; "PDC PLSNO"; Text[30])
        {
            Caption = 'PLSNO';
        }
        field(50007; "PDC Consignment No."; Code[20])
        {
            Caption = 'Consignment No.';
        }
        field(50011; "PDC Pick Instruction Print No."; Integer)
        {
            Caption = 'Pick Instruction Print No.';
        }
        field(50012; "PDC Notes"; Text[250])
        {
            Caption = 'Notes';
        }
        field(50013; "PDC Branch No."; Code[20])
        {
            Caption = 'Branch No.';
        }
        field(50014; "PDC Order Source"; Code[20])
        {
            Caption = 'Order Source';
            TableRelation = "PDC General Lookup_existing".Code where(Type = const(Source));
        }
        field(50015; "PDC Order Status"; Code[20])
        {
            Caption = 'Order Status';
            TableRelation = "PDC General Lookup_existing".Code where(Type = const(Status));
        }
        field(50016; "PDC Checked By"; Code[50])
        {
            Caption = 'Checked By';
            Editable = false;
            TableRelation = User;
            ValidateTableRelation = false;
        }
        field(50017; "PDC Last Picking No."; Code[10])
        {
            Caption = 'Last Picking No.';
            Editable = false;
            TableRelation = "Warehouse Activity Header"."No." where("Source No." = field("No."));
            ValidateTableRelation = false;
        }
        field(50018; "PDC CCM Notes"; Text[250])
        {
            Caption = 'CCM Notes';
        }
        field(50019; "PDC Warehouse Document No."; Code[20])
        {
            CalcFormula = lookup("Warehouse Activity Header"."No." where("Source Document" = filter("Sales Order"),
                                                                          "Source No." = field("No.")));
            Caption = 'Warehouse Document No.';
            Editable = false;
            FieldClass = FlowField;
        }
        field(50028; "PDC Roll-Out No."; Code[20])
        {
            Caption = 'Roll-Out Number';
            TableRelation = "PDC Roll-out".Code;
        }
        field(50032; "PDC Ship-to E-Mail"; Text[80])
        {
            Caption = 'Ship-to E-Mail';
        }
        field(50033; "PDC Home Address"; Boolean)
        {
            Caption = 'Home Address';
        }
    }

    /// <summary>
    /// SendEmail.
    /// </summary>
    procedure SendEmail()
    var
        Customer: Record Customer;
        SalesReceivablesSetup: Record "Sales & Receivables Setup";
        PDCEmailManagementSetup: Record "PDC Email Management Setup";
        TempBlob: Codeunit "Temp Blob";
        RecRef: RecordRef;
        OutStr: OutStream;
        FileName: Text[250];
        EmailStr: Text;
    begin
        if not Confirm('Do you want to send the Invoice ?') then
            Error('');

        SalesReceivablesSetup.Get();
        PDCEmailManagementSetup.Get(SalesReceivablesSetup."PDC Invoice E-mail Setup", PDCEmailManagementSetup.Type::Setup, 0);

        if ReportSelections.Get(ReportSelections.Usage::"S.Invoice", 1) then
            ReportNo := ReportSelections."Report ID"
        else
            ReportNo := 206;

        Customer.Get("Bill-to Customer No.");

        if Customer."PDC Send Statement" then
            if (Customer."E-Mail" <> '') or (Customer."PDC Invoice E-Mail" <> '') then begin

                FileName := 'Invoice_' + "No." + '.pdf';

                TempBlob.CreateOutStream(OutStr);

                Rec.SetRecfilter();
                RecRef.GetTable(Rec);
                Report.SaveAs(ReportNo, '', ReportFormat::Pdf, OutStr, RecRef);

                if Customer."PDC Invoice E-Mail" <> '' then
                    EmailStr := Customer."PDC Invoice E-Mail"
                else
                    EmailStr := Customer."E-Mail";

                PDCEmailManagementSetup.ProcessEmail(PDCEmailManagementSetup.Code, CopyStr(EmailStr, 1, 80), Customer.Name, "No.", 'Peter Drew Contracts Ltd', FileName, TempBlob, "Sell-to Customer No.", Customer."PDC Invoice Mailing Group");
                Message('The email was sent successfully');
            end else
                Error('Customer is not set to send emails.');
    end;

    /// <summary>
    /// SendEmailSilent.
    /// </summary>
    procedure SendEmailSilent()
    var
        Customer: Record Customer;
        SalesReceivablesSetup: Record "Sales & Receivables Setup";
        PDCEmailManagementSetup: Record "PDC Email Management Setup";
        EmailStr: Text;
    begin
        SalesReceivablesSetup.Get();
        PDCEmailManagementSetup.Get(SalesReceivablesSetup."PDC Invoice E-mail Setup", PDCEmailManagementSetup.Type::Setup, 0);

        Customer.Get("Bill-to Customer No.");
        if Customer."PDC Send Invoice" then
            if (Customer."E-Mail" <> '') or (Customer."PDC Invoice E-Mail" <> '') then begin
                if Customer."PDC Invoice E-Mail" <> '' then
                    EmailStr := Customer."PDC Invoice E-Mail"
                else
                    EmailStr := Customer."E-Mail";
                PDCEmailManagementSetup.ProcessEmail(PDCEmailManagementSetup.Code, CopyStr(EmailStr, 1, 80), Customer.Name, "No.", 'Peter Drew Contracts Ltd', Rec.RecordId, "Sell-to Customer No.", Customer."PDC Invoice Mailing Group");
            end;
    end;

    var
        ReportSelections: Record "Report Selections";
        ReportNo: Integer;
}

